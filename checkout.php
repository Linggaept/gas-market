        ...
        <div class="row mb-3">
                    ...
                    <div class="card-body">
                        ...
                        <form action="" method="post" id="checkoutForm">
                            ...
                            <div class="form-group">
                                ...
                                <select name="city_destination" id="city_destination" class="form-control" onchange="get_district_destination(this);" required>
                                    <option value=""></option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label class="font-weight-bold">Kecamatan</label>
                                <select name="district_destination" id="district_destination" class="form-control" required>
                                    <option value=""></option>
                                </select>
                            </div>
                            <div class="form-group">
            ...
            <div class="col-md-6">
                ...
                if (isset($_POST['pesan'])) {
                    ...
                    $ongkir = $_POST['ongkir'];
                    $totalbayar = (int)$_POST['subtotal'] + (int)$_POST['ongkir'];

                    // Validasi ongkir
                    if ($ongkir === '' || !is_numeric($ongkir) || $ongkir == 0) {
                        echo "<script>alert('Silakan pilih lokasi pengiriman dan tunggu ongkir muncul!');window.history.back();</script>";
                        exit;
                    }
...
<script>
    ...
    function get_city_destination(sel) {
        $.getJSON("assets/checkout/city.php?id=" + sel.value, function(get_city) {
            if (get_city) {
                ...
                $("#district_destination").html("<option value=''>Pilih Kecamatan</option>");
                $("#ongkir").html("");
                $("#total").html("");
                $("#ongkir_value").val("");
                $.each(get_city['rajaongkir']['results'], function(key, value) {
                    $("#city_destination").append(
                        "<option value='" + value.city_id + "'>" + (value.type ? value.type + " - " : "") + value.city_name + "</option>"
                    );
                });
            }
        });
    ...
    function get_district_destination(sel) {
        $.getJSON("assets/checkout/district.php?id=" + sel.value, function(get_district) {
            if (get_district) {
                $("#district_destination").html("<option value=''>Pilih Kecamatan</option>");
                $("#ongkir").html("");
                $("#total").html("");
                $("#ongkir_value").val("");
                $.each(get_district['rajaongkir']['results'], function(key, value) {
                    $("#district_destination").append(
    ...
    if (!ongkir || isNaN(ongkir) || ongkir == "0") {
        alert('Silakan pilih lokasi pengiriman dan tunggu ongkir muncul sebelum checkout!');
        e.preventDefault();
    }
...
<script type="text/javascript">
    $("#district_destination").change(function() {
        var subtotal = <?php echo $subtotal; ?>;
        var district_id = $(this).val();

        if (district_id) {
            $("#ongkir").html("Menghitung...");
            $.ajax({
                url: "assets/checkout/ongkir.php",
                type: "GET",
                data: {
                    district_destination: district_id
                },
                success: function(response) {
                    var ongkir = parseInt(response);
                    if (!isNaN(ongkir) && ongkir > 0) {
                        $("#ongkir_value").val(ongkir);
                        $("#ongkir").html(ongkir.toLocaleString('id-ID'));
                        ...
                        var total = subtotal;
                        $("#total").html(total.toLocaleString('id-ID'));
                        alert("Ongkir tidak tersedia untuk tujuan ini.");
                    }
